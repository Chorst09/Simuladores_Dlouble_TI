...
const FiberLinkCalculator: React.FC<FiberLinkCalculatorProps> = ({
  userRole = 'user',
  onBackToPanel = () => { },
  userId = '',
  userEmail = ''
  ...
  const [clientData, setClientData] = useState<ClientData>({ name: '', projectName: '', email: '', phone: '' });
  ...
  useEffect(() => {
    ...
    const fetchProposals = async () => {
      ...
      try {
        const response = await fetch('/api/proposals?type=FIBER_LINK', {
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
          },
  ...
  // ... (installationTiers and contractTerms remain the same) ...
  ...
  const handleAddProduct = () => {
    if (result) {
      ...
      const description = `Internet Fibra ${result.plan.description || 'Plano'} - Contrato ${contractTerm} meses${includeInstallation ? ' (com instalação)' : ''}`;

      setAddedProducts(prev => [...prev, {
        id: generateUniqueId(),
        name: `Internet Fibra ${result.plan.description || 'Plano'}`,
        description,
  ...
  // CORREÇÃO: Bloco de código solto removido daqui. A lógica foi incorporada em handleNewProposal.
  ...
  const handleSaveProposal = async () => {
    ...
    const storageKey = 'fiberProposalCounter';
    ...
    const proposalNumber = `FL-${formattedNumber}/${year}${version}`;
    ...
    return (
      <div className="p-4 md:p-8">
        <ClientManagerForm
          ...
          title="Nova Proposta de Link de Fibra"
    ...
    return (
      <div className="p-4 md:p-8 print-hide">
        <Card className="bg-slate-900/80 border-slate-800 text-white">
          <CardHeader>
            <CardTitle>Buscar Propostas - Internet Fibra</CardTitle>
            <CardDescription>Encontre propostas de Internet Fibra existentes ou crie uma nova.</CardDescription>
          </CardHeader>
          <CardContent>
  ...
  return (
    <div className="p-4 md:p-8 print-hide">
      <Card>
        <CardHeader>
          <CardTitle>Calculadora de Link de Fibra</CardTitle>
          <CardDescription>
            Configure os parâmetros do link de fibra e visualize os valores
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="mb-6">
                ...
                <Card className="bg-slate-900/80 border-slate-800 text-white">
                  <CardHeader>
                    <CardTitle className="flex items-center">
                      <Wifi className="mr-2" />Internet Fibra
                    </CardTitle>
                  </CardHeader>
                  <CardContent>
            ...
            {(userRole === 'admin' || userRole === 'diretor') && (
              <TabsContent value="list-price">
                <Card className="bg-slate-900/80 border-slate-800 text-white mt-6">
                  <CardHeader>
                    <CardTitle>Tabela de Preços - Internet Fibra</CardTitle>
