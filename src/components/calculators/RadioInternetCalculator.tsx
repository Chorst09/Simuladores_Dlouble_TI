...
const RadioInternetCalculator: React.FC<RadioInternetCalculatorProps> = ({ userRole, onBackToPanel, userId, userEmail }) => {
    ...
    useEffect(() => {
        const fetchProposals = async () => {
            try {
                ...
                const response = await fetch('/api/proposals?type=RADIO_INTERNET', {
                    headers: {
    ...
    useEffect(() => {
        if (directorDiscountData) {
            setFinalTotalMonthly(directorDiscountData.discountedValue);
        } else {
            setFinalTotalMonthly(totalMonthly);
        }
    }, [totalMonthly, directorDiscountData]);

    const clearForm = () => {
        setClientData({ name: '', projectName: '', email: '', phone: '' });
    ...
    const handleSaveNegotiation = async (updatedProposal: Proposal) => {
        try {
            console.log('Salvando negociação:', updatedProposal);
            ...
            const requestData = {
                ...
                total_setup: Number(updatedProposal.totalSetup) || 0,
                total_monthly: Number(updatedProposal.totalMonthly) || 0,
                ...
                status: updatedProposal.status || 'Pendente',
                type: 'RADIO_INTERNET'
            };
    ...
    const editProposal = (proposal: Proposal) => {
        setCurrentProposal(proposal);
        setClientData(proposal.clientData || { name: '', projectName: '', email: '', phone: '' });
    ...
    const saveProposal = async () => {
        ...
        const proposalToSave = {
            ...
            total_monthly: finalTotalMonthly,
            created_at: new Date().toISOString(),
            user_id: userId,
            user_email: userEmail,
            status: 'pending',
            type: 'RADIO_INTERNET',
            director_discount: directorDiscountData || null,
            negotiation_rounds: [],
            current_round: 1
        };
                                            ...
                                            <div className="space-y-6">
                                                ...
                                                {(userRole === 'user' || userRole === 'admin' || userRole === 'diretor') && (
                                                    <div className={`border border-slate-600 rounded-lg p-4 ${userRole === 'diretor' ? 'bg-slate-800/50 opacity-75' : 'bg-slate-800'}`}>
                                                        ...
                                                        <p className="text-sm text-slate-400">Desconto de 5%</p>
                                                        ...
                                                        <Button 
                                                            ...
                                                            disabled={negotiationRound !== 1 || userRole === 'diretor'}
                                                        >
                                                ...
                                                {discount > 0 && (
                                                    <div className="border border-slate-600 rounded-lg p-4 bg-slate-800">
                                                        <h4 className="text-lg font-semibold text-green-400 mb-4">Resumo do Desconto Aplicado</h4>
                                                        <p className="text-slate-300">Desconto de <span className="font-bold text-orange-400">{discount}%</span> aplicado.</p>
                                                        <p className="text-sm text-slate-400">Valor final:</p>
                                                        <p className="text-2xl font-bold text-orange-400">{formatCurrency(getDiscountedPrice())}</p>
                                                        {negotiationRound === 3 && <p className="text-sm text-orange-500">Negociação finalizada pelo diretor.</p>}
                                                    </div>
                                                )}
